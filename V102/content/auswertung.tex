...
\centering
\input{Tabellen/Draht_Eigenschaften}

\end{minipage}
\hspace{5pt}
\begin{minipage}[t]{0.4\textwidth}
\centering
\input{Tabellen/Kugel_Eigenschaften}
...
\begin{alignat}{3}
	\Theta	&=\Theta_\text{Kugel} &&+\Theta_\text{Aufhängung}\label{eq:gesamttraegheit}\\ 
		&=\frac{2}{5} m_\text{K} r_\text{K}^2 &&+\Theta_\text{Aufhängung} %+\SI{22.5e-7}{\kilo\gram\meter\squared}
\end{alignat}
...
\input{Tabellen/Dipol_StromZeit.tex}
...
\input{Tabellen/Erdmagnet_Zeit.tex}
